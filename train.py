...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if (
        args.algorithm in STATIC_GRAPH_ALGORITHMS
        and isinstance(model.model, torch.nn.parallel.DistributedDataParallel)
        and hasattr(model.model, "_set_static_graph")
    ):
        model.model._set_static_graph()
        logger.info(f"Enabled DDP static graph mode for {args.algorithm}")
